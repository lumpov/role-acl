...
import { compareGT } from './compareUtil';

export class GTCondition implements IConditionFunction {
    evaluate(args?: any, context?: any) {
        ...
        if (CommonUtil.type(args) !== 'object') {
            throw new AccessControlError('GTCondition expects type of args to be object');
        }
        return Object.keys(args).every((key) => {
            ...
            return compareGT(keyValue, argValue);
        });
    }
}
